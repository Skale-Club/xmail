...
- ✅ **DONE — Migrations 038–043 APPLIED to production** (2026-07-17, via a targeted `postgres.js` runner, prepare:false, against the aws-1-us-east-2 pooler). All 14 v1.4 tables + key columns verified present; versions registered in `supabase_migrations.schema_migrations`. Prod had 0 campaigns/0 sequences so 040's merge was a no-op. ~~NOTE: a pre-existing tracking drift remains — migrations 023–037 are applied to the schema but unregistered in `schema_migrations`.~~ **RESOLVED 2026-08-13:** the ledger drift is gone. `scripts/apply-pending-migrations.mjs` was run against the full pending set (023–037, 044–050) after auditing every file for idempotency; each one reported `already exists, skipping`, confirming the schema was already correct and only the ledger was behind. `supabase_migrations.schema_migrations` now records through `050` and the runner reports nothing pending. The earlier "do NOT run the naive runner" warning no longer applies — but note the runner still cannot handle `CREATE INDEX CONCURRENTLY` (it wraps each file in a transaction); see the migration section in `CLAUDE.md`.
...
- ✅ **DONE — `XMAIL_SERVICE_USER_ID` + `XMAIL_SERVICE_ORGANIZATION_ID`** set as GitHub secrets on 2026-08-11, and both are wired into `run_app_container()` in `build-deploy.yml`. Outreach machine auth (Xphere orchestrator) is enabled.
- ⏳ `XPHERE_EVENTS_API_KEY` — **not set**. `XPHERE_EVENTS_URL` alone is not enough: `deliverOutreachEvents` no-ops unless both are present, so outbound Xphere event delivery is still off. Events keep accumulating durably in `outreach_event_outbox` (nothing is lost), and Hermes consumes them independently via its own cursor.
- ⏳ `APOLLO_API_KEY` — **not set**. Apollo prospecting (`/api/agent/outreach/prospecting/*`) returns a configuration error until it is. The `-e APOLLO_API_KEY` wiring was missing from the active `build-deploy.yml` (it existed only in the legacy `deploy-hetzner.yml`) and was added on 2026-08-13; setting the secret is now sufficient.
