...
- [ ] **CRIT-02**: `/health/ready` returns HTTP 503 with `database.ok=false` when the database is unreachable or returning errors. (audit C3)
- [ ] **CRIT-03**: `POST /api/mail/mailboxes/test-connection` requires authenticated user (`userId` from Supabase token), rejects private/loopback hosts, and is covered by a per-user rate limit. (audit C1)
...
- [ ] **COR-01**: `POST /webhooks/:id/test` uses `AbortSignal.timeout(10_000)`. (audit H2)
- [ ] **COR-02**: `fireWebhooks` retries failed deliveries with exponential backoff (1s/3s/9s, max 3 attempts) and persists each attempt to `webhook_requests` with an `attempts` counter. (audit M6, H2)
