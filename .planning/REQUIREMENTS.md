...
- [x] **QUA-01**: `npx tsc --noEmit` passes with zero errors across both `tsconfig.json` and `tsconfig.server.json` (fix `AppLogo.tsx:12` unused var and `tracking.ts:266` `event as any` cast). (audit M1, M12)
...
- [x] **QUA-04**: `POST /api/domains` lowercases and trims `name` before insert. A one-shot data migration lowercases existing `domains.name`. (audit M9)
