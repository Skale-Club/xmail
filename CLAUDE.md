...
**Numbering convention:** Migrations are sequential integers. As of 2026-08-13 the highest *applied* is `050_campaign_content_language.sql`; `051`–`053` exist in the repo but are NOT yet applied, so the next free number is `054`. When two phases plan migrations in parallel, the second to land takes the next number and rewrites its planning docs accordingly.
