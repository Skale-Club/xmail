...
**Numbering convention:** Migrations are sequential integers. As of 2026-08-14 production is applied through `056_prospecting_external_run_id.sql`; the next free number is `057`. When two phases plan migrations in parallel, the second to land takes the next number and rewrites its planning docs accordingly.
