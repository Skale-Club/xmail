...
| 13 | ~~Média~~ | ~~O Xphere não enviava `websiteInsights` ao Xmail~~ — **resolvido**: o enrolamento carrega o insight multilíngue da análise por conta e o inclui em `customFields` | — |
| 14 | ~~Média~~ | ~~`prospects_list` não expunha presença, booking, localização nem telefone~~ — **resolvido em 2026-09-05**: o MCP devolve os campos por prospect, resumo agregado e filtros `no_owned_website`, tipo exato e plataforma | — |
| 15 | ~~Média~~ | ~~O Xphere descartava `enrichedCount` e `coverage`~~ — **resolvido em 2026-09-05**: o Xcraper mede, o Xphere valida/mapeia e o Xmail persiste; valores não medidos continuam ausentes | — |
